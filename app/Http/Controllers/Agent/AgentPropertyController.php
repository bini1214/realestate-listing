...
$amen = $request->amenities_id;
    // Ensure $amen is an array
    $amen = is_array($amen) ? $amen : explode(',', $amen);

    // Implode the array of amenity IDs
...
if ($image) {
    $name_gen = hexdec(uniqid()) . '.' . $image->getClientOriginalExtension();
    // Further operations with $image...
} else {
    // Handle cases where no image is uploaded, perhaps set a default or just skip processing
    $name_gen = 'default.png'; // Example default, adjust as necessary
}

        //Image::make($image)->resize(370,250)->save('upload/property/thambnail/'.$name_gen);
        //$image = ImageManager::imagick()->read('images/example.jpg');
        //$image->resize(370,250)->save('upload/property/thambnail/'.$name_gen);
    ...
    $defaultImage = 'default_image.jpg'; // Replace with your default image path
    MultiImage::insert([
        'property_id' => $property_id,
        'photo_name' => $defaultImage,
        'created_at' => Carbon::now(),
    ]);
}// End Foreach
...
public function AgentUpdatePropertyThambnail(Request $request){
    ...
    $img->resize(770,520)->save('upload/property/multi-image/'.$make_name);;
        $uploadPath = 'upload/property/multi-image/'.$make_name;
    ...
    //$image = ImageManager::imagick()->read('images/example.jpg');
    $image->resize(770,520)->save('upload/property/multi-image/'.$make_name);
        $uploadPath = 'upload/property/multi-image/'.$make_name;
